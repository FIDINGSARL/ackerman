				...
				setTimeout(function() {
					...
					if(all_quotes_length == 0){
						...
						if($(".floor-screen.screen").is(":visible")){
							if(self.env.pos.config.floor_ids && self.env.pos.config.floor_ids.length){
								let wkFloorId = self.env.pos.config.floor_ids[0]
								if(self.env.pos.floors_by_id[wkFloorId] && self.env.pos.floors_by_id[wkFloorId].table_ids && self.env.pos.floors_by_id[wkFloorId].table_ids.length){
									let wkTableId = self.env.pos.floors_by_id[wkFloorId].table_ids[0]
									if (self.env.pos.tables_by_id[wkTableId]){
										self.env.pos.set_table(self.env.pos.tables_by_id[wkTableId])
		...
		mounted(){
			...
			if(!(self.props && self.props.quote_status)){
				setTimeout(function(){
					...
					self.cancel();
					if(!$(".product-screen.screen").is(':visible')){
